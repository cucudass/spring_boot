...
public class UploadServiceImpl implements UploadService {
	...
	public void deleteFiles(List<BoardAttachDTO> fileList) {
		log.info("@# UploadServiceImpl deleteFiles()");
		...
		fileList.forEach(attach -> {
			try {
				...
				Files.deleteIfExists(file); //존재 시 삭제
				
				//이미지일때 썸네일 삭제
				if(Files.probeContentType(file).startsWith("image")) {
					Path thumNail = Paths.get("D:\\dev\\upload\\"+attach.getUploadPath()+"\\s_"+attach.getUuid()+"_"+attach.getFileName());
					Files.deleteIfExists(thumNail); //존재 시 삭제
				}
			} catch (Exception e) {
